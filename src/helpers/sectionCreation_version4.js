...
export async function sectionCreationV4(gridsValue, type="v4"){
    ...
    if(type === 'v4'){
        ...
        // obj = await createGridsV8(gridsValue, obj);
        // const values = await generateGridData(gridsValue);
        // console.log(values);
        // obj = await createGridsV9(gridsValue, obj);
    }
    // obj = await createGridsV5(gridsValue, obj);
        ...
        Object.entries(values).forEach(([inlineKey, inlineValue], InlineIndex)=>{
            ...
            Object.entries(inlineValue?.blocks).forEach(([blockKey, blockValue], blockIndex)=>{
                ...
                let cumulativeInlineSize = 0,
                    ...
                    additionBlockSize = true,
                    additionInlineSize = true,
                ...
                if(gridArr.length){
                    ...
                    gridArr.forEach((gridEle, gridIndex)=>{
                        let currentChildBlockSize = gridEle?.size?.block,
                        currentChildInlineSize = gridEle?.size?.inline; 
                    const IsBlockChildEle = currentChildBlockSize < currentBlockSize;
                    const IsInlineChildEle = gridEle?.IsInlineChildEle;
            
                    // if(cumulativeInlineSize + currentChildInlineSize > currentInlineSize) cumulativeInlineSize = 0;
                    if(cumulativeBlockSize + currentChildBlockSize > currentBlockSize) cumulativeBlockSize = 0;
                    if(IsBlockChildEle && IsInlineChildEle){
                        if(cumulativeBlockSize > 0){
                            cumulativeInlineSize -= currentChildInlineSize;
                        }
                        console.log(`BlockSize`, cumulativeBlockSize)
                        cumulativeInlineSize += currentChildInlineSize;
                        if(additionBlockSize && BlockEleCount === 1) cumulativeBlockSize += currentChildBlockSize;
                        BlockEleCount = BlockEleCount + 1;
                        // cumulativeBlockSize += currentChildBlockSize;
                    }
                    else if(!IsBlockChildEle && IsInlineChildEle){
                        // if(cumulativeInlineSize + currentChildInlineSize > currentInlineSize) cumulativeInlineSize = 0;
                        // if(cumulativeBlockSize + currentChildBlockSize > currentBlockSize) cumulativeBlockSize = 0;
                        BlockEleCount = 1;
                        if(cumulativeInlineSize > 0){
                            cumulativeBlockSize -= currentChildBlockSize;
                        }
                        cumulativeInlineSize += currentChildInlineSize;
                        cumulativeBlockSize += currentChildBlockSize;
                        ...
                        //     BlockEleCount = 1;
                        // }
                        // console.log(cumulativeBlockSize)
                        if(cumulativeBlockSize > 0){
                            cumulativeInlineSize -= currentChildInlineSize;
                        }
                        cumulativeInlineSize += currentChildInlineSize;
                        ...
                        // if(IsInlineChildEle){
                        //     const parentInlineSize = gridEle?.InlineChildSize;
                        //     if(internalInlineSize > 0) additionBlockSize = false;
                        //     internalInlineSize += currentChildInlineSize;
                        //     if(IsBlockChildEle && internalInlineSize >= parentInlineSize){
                        //         BlockEleCount = BlockEleCount + 1;
                        //     }
                        // }
                        // else if(IsBlockChildEle && !IsInlineChildEle){
                        ...
                        // }
                        // cumulativeInlineSize += currentChildInlineSize;
                        // if(additionBlockSize) cumulativeBlockSize += currentChildBlockSize;
                        console.log(cumulativeInlineSize, cumulativeBlockSize);
                        // if(cumulativeBlockSize >= currentBlockSize) cumulativeBlockSize = 0;
                        // if(cumulativeInlineSize >= currentInlineSize) cumulativeInlineSize = 0;
                    })
...
async function createGridsV8(values, obj) {
    if (values) {
        Object.entries(values).forEach(([inlineKey, inlineValue]) => {
            let currentInlineSize = inlineValue?.size;

            Object.entries(inlineValue?.blocks).forEach(([blockKey, blockValue]) => {
                let currentBlockSize = blockValue?.size;
                let cumulativeInlineSize = 0,
                    cumulativeBlockSize = 0,
                    cumulativeInternalInlineSize = 0,
                ...
                // console.log(gridArr);
                calculateCumulativeSizes(gridArr, currentInlineSize, currentBlockSize);
            });
        });
    }
    return obj;
...
function calculateCumulativeSizes(gridArr, currentInlineSize, currentBlockSize) {
    let cumulativeInlineSize = 0,
        cumulativeBlockSize = 0,
        cumulativeInternalInlineSize = 0,
        addtionInlineWidth = true,
    ...
    for (let ga = 0; ga < gridArr.length; ga++) {
        ...
        console.log(`Cumulative inline size: ${cumulativeInlineSize}, Cumulative block size: ${cumulativeBlockSize}`);
    }
}
...
async function generateGridData(object) {
    const columns = [];
    const rows = [];
    const areas = [];
    
    // Helper function to recursively process the object
    function processBlock(block, colStart, rowStart) {
        let colSpan = 1;
        let rowSpan = 1;
        ...
        if (block.size) {
            if (block.size.includes('/')) {
                [colSpan, rowSpan] = block.size.split('/').map(Number);
            } else {
                // Convert size to a percentage for columns or rows
                if (Array.isArray(block.size)) {
                    columns.push(`${block.size[0] / block.size[1] * 100}%`);
                    rows.push(`${block.size[0] / block.size[1] * 100}%`);
                } else {
                    columns.push(`${block.size}%`);
                    rows.push(`${block.size}%`);
                }
            }
        }

        // Calculate the area string
        areas.push(`${block.name} ${colStart} / ${colStart + colSpan} / ${rowStart} / ${rowStart + rowSpan}`);
        ...
        if (block.children) {
            ...
            for (const [key, child] of Object.entries(block.children)) {
                processBlock(child, childColStart, childRowStart);
                
                // Move to next column/row
                childColStart += 1;
                if (childColStart > 3) { // Assuming fixed 3 columns
                    childColStart = colStart;
                    childRowStart += 1;
                }
            }
        }
    ...
    for (const [key, block] of Object.entries(object.blocks)) {
        processBlock(block, 1, 1);
    }
    
    // Construct grid template strings
    const gridTemplateColumns = `grid-template-columns: ${columns.join(' ')};`;
    const gridTemplateRows = `grid-template-rows: ${rows.join(' ')};`;
